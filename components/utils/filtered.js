...
function getDates(data, startDate, stopDate) {
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        if(moment(data[i].createdAt.slice(0,10)).isSameOrAfter(startDate) && moment(data[i].createdAt.slice(0,10)).isSameOrBefore(stopDate)) {
            ndata.push(data[i]);
        }
    }
...
function getDatesInTime(data, startDate, stopDate) {
    ...
    for(let i = 0; i < data.length; i++) {
        if(moment(data[i].date2.slice(0,10)).isSameOrAfter(startDate) && moment(data[i].date2.slice(0,10)).isSameOrBefore(stopDate)) {
            ndata.push(data[i]);
        }
    }
