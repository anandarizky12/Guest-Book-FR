...
function AdminDataPrint( { admin, filter, setFilter, activePage, setActivePage} ) {
    ...
    return (
                <div className='flex flex-col w-full p-5 items-center my-2'>
                    ...
                    <header className='text-sm'>DATA ADMIN PADA BADAN PUSAT STATISTIK KOTA BANJARMASIN</header>
                                    ...
                                    {admin &&  paginationData(getFiltered(admin.adminData , filter), activePage, 10).map((admin , index) => (
                                        <tr key={index} className ="border-b border-gray-200 hover:bg-gray-100">
                                            ...
                                            <td className ="py-3 px-6 text-left">
                                                <div className ="flex items-center">
                                                    <span className ="bg-red-500 text-gray-100 py-1 px-3 rounded-full text-sm">{admin.email}</span>
                                                </div>
                                            </td>
