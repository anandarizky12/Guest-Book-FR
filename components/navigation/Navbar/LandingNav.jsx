...
function LandingNav({ about, contact, home }) {
  ...
  return (
    <div
      ...
      } h-12 fixed sm:h-16 z-50 flex w-full justify-between items-center p-4 sm:px-6`}
    >
      <div className="bg-none top-0 p-0 sm:p-5 text-blue-800 flex items-center text-base md:text-xl font-semibold">
        <img
          alt="buku tamu bps"
          src="/onlylogo.png"
          className="mr-1 md:mr-2 w-12 md:w-18"
        />
      ...
      <div ref={ref}>
        ...
        <ul
          className={`${
            show
              ? "h-80 transition-all duration-100"
        ...
        >
          <li className="m-6 cursor-pointer" onClick={() => handleNav(home)}>
            Home
          </li>
          <li className="m-6 cursor-pointer" onClick={() => handleNav(about)}>
            Tentang
          </li>
          <li className="m-6 cursor-pointer" onClick={() => handleNav(contact)}>
            Contact
          </li>
          <li
            onClick={() => Router.push("/login")}
            className="cursor-pointer m-6 hover:bg-green-400 bg-green-600 px-4 p-1 rounded-md flex items-center text-white"
          >
      ...
      {/* resposive desktop*/}
      <ul
        className={`w-fullease-in  hidden  text-gray-500 top-12 md:flex font-semibold items-center`}
      >
        ...
        <li onClick={() => handleNav(contact)} className="mr-5 cursor-pointer">
          Contact
        </li>
        <li
          onClick={() => Router.push("/login")}
          className="mr-5 cursor-pointer hover:bg-green-400 bg-green-600 px-4 p-1 rounded-md flex items-center text-white"
        >
          <FaSignInAlt className="mr-2" /> Admin
        </li>
      </ul>
